...
from conan.tools.files import apply_conandata_patches, export_conandata_patches, get, replace_in_file, rm, rmdir, copy, save, rename
...
class SDLConan(ConanFile):
    ...
    def _patch_sources(self):
        ...
        if Version(self.version) >= "2.0.0":
            ...
            rename(self, os.path.join(self.source_folder, "include", "SDL_config.h.default"), os.path.join(self.source_folder, "include", "SDL_config.h"))
...
list(APPEND SDL_SOURCES "src/thread/generic/SDL_syscond.c")

message(WARNING "${SDL_SOURCES}")
...
target_link_libraries(${PROJECT_NAME} dxguid.lib Winmm.lib)
...
""")
    ...
    def package_info(self):
        ...
        if Version(self.version) >= "2.0.0":
            ...
            self.cpp_info.set_property("cmake_file_name", "SDL")

            self.cpp_info.names["cmake_find_package"] = "SDL"
            self.cpp_info.names["cmake_find_package_multi"] = "SDL"
            ...
            self.cpp_info.components["libsdl"].set_property("cmake_target_name", "SDL::SDL")
            if not self.options.shared:
                self.cpp_info.components["libsdl"].set_property("cmake_target_aliases", ["SDL::SDL-static"])
            self.cpp_info.components["libsdl"].set_property("pkg_config_name", "sdl")

            sdl_cmake_target = "SDL" if self.options.shared else "SDL-static"
            self.cpp_info.components["libsdl"].names["cmake_find_package"] = sdl_cmake_target
            self.cpp_info.components["libsdl"].names["cmake_find_package_multi"] = sdl_cmake_target

            self.cpp_info.components["libsdl"].libs = ["SDL" + lib_postfix]

            if self.settings.os == "Windows":
                self.cpp_info.components["libsdl"].system_libs = ["user32", "gdi32", "winmm", "imm32", "ole32", "oleaut32", "version", "uuid", "advapi32", "setupapi", "shell32"]
                if self.settings.compiler == "gcc":
                    self.cpp_info.components["libsdl"].system_libs.append("mingw32")
