...
class SDLImageConan(ConanFile):
    ...
    def source(self):
        ...
        if Version(self.version) < "2.0.0":
            save(self, os.path.join(self.source_folder, os.pardir, "conanfile.txt"), """[requires]
sdl/1.2.15
...
CMakeToolchain
""")
            save(self, os.path.join(self.source_folder, os.pardir, "CMakeLists.txt"), """cmake_minimum_required(VERSION 3.1)
project(sdl_image)
...
install(FILES {dir}/SDL_image.h DESTINATION ${{CMAKE_INSTALL_INCLUDEDIR}})
""".format(dir="src"))
    ...
    def package_info(self):
        if Version(self.version) >= "2.0.0":
            ...
            self.cpp_info.set_property("cmake_file_name", "SDL_image")
            self.cpp_info.set_property("cmake_target_name", "SDL_image::SDL_image")
            if not self.options.shared:
                self.cpp_info.set_property("cmake_target_aliases", ["SDL_image::SDL_image-static"])
            self.cpp_info.set_property("pkg_config_name", "SDL_image")
            # TODO: back to global scope in conan v2 once legacy generators removed
            self.cpp_info.components["_sdl_image"].libs = ["SDL_image"]

            # TODO: to remove in conan v2 once legacy generators removed
            self.cpp_info.names["cmake_find_package"] = "SDL_image"
            #self.cpp_info.names["cmake_find_package_multi"] = "SDL_image"
            self.cpp_info.names["pkg_config"] = "SDL_image"
            target_name = "SDL2_image" if self.options.shared else "SDL_image-static"
            self.cpp_info.components["_sdl_image"].names["cmake_find_package"] = target_name
            #self.cpp_info.components["_sdl_image"].names["cmake_find_package_multi"] = target_name
            self.cpp_info.components["_sdl_image"].set_property("cmake_target_name", "SDL_image::SDL_image")
            self.cpp_info.components["_sdl_image"].set_property("pkg_config_name", "SDL_image")
            self.cpp_info.components["_sdl_image"].requires = ["sdl::sdl"]
